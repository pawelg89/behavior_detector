...
public:
	//Zmienne w których umieszczony zostanie po³o¿enie na p³aszczyŸnie po której porusza siê obiekt.
	double X;
	double Z;
	double obj_h;
	...
	Helper(double w, double h, double fo, double Ha, double alfaa, int cam_w, int cam_h);
	...
	cam_w - resolution width part, number of pixels in line,
	cam_h - resolution height part, number of pixels in column*/
	Helper(double dx, double dy, double fo, double Ha, double alfaa, int cam_w, int cam_h,bool test);
	~Helper(void);
	...
	double GetFocal();
	double GetCamHeight();
	void Set(double dx, double dy, double fo, double Ha, double alfaa, int camW, int camH);
};