...
//---------------------------------------------------------------------------------------------------------------------
//---------------------------N I E -- D O T Y K A Æ ! -- G R O Z I -- Œ M I E R C I ¥ ---------------------------------
//---------------------------------------------------------------------------------------------------------------------
...
double Convex::GetAlfa(Point p1, Point prePt)
{
	...
	if( dx > 0 && dy > 0)
		...
		return 0;
  else throw std::runtime_error("I did not expect this!");
}
...
int Convex::UpdateHull(int startHull, int endHull, vector<Point> &hull, vector<Point> &contour,int NCMidx)
{
	vector<Point> temp_hull;
	int nbr_ptsAdded = 0;
	bool correct = true;

	//------------------------EXTRACT SUBVECTOR OF CONTOUR----------------------------------------
	int idxStart=0;
	for(idxStart; idxStart < (int)contour.size(); idxStart++)
		if(contour[idxStart] == hull[startHull])
			break;
		
	int idxStop=0;
	for(idxStop; idxStop < (int)contour.size(); idxStop++)
		if(contour[idxStop] == hull[endHull])
			break;

	vector<Point> subVec;
	if(idxStart < idxStop)
		for(int i = idxStart; i <= idxStop; i++)
			subVec.push_back(contour[i]);
	else
	...
	//circle(drawing, subVec[subVec.size() - 1],0,cvScalar(0,255,0),1);//green stop
	...
	//------------------------FIND NCM IN CURRENT SUBVECTOR---------------------------------------
	int ncmidx_subv = -1;
	for (int i=0; i < (int)subVec.size(); i++)
		if (subVec[i] == contour[NCMidx])
			ncmidx_subv = i;
	//------------------------CALCULATE HULL POINTS UNTIL NCM------------------------------------
	Point prePt;
	Point p1;
	Point p2 = start;
	if (idxStart > 0)
	...
	do{
		...
		Point temp_p2;
		int it = 0;
		//Przesun iterator na aktualny punkt, zeby nastepny for wyszukiwal tylko wsrod punktow 
		//znajdujacych sie za p1 ale przed NCM'em
	...
	//--------------------CALCULATE HULL POINTS FROM NCM TO HULL_STOP-----------------------------
	...
	//------------LAST ADDED POINT WAS SAME AS 'end' SO WE NEED TO REMOVE IT--------------------
		
	for(int ithull = 0; correct && ithull < static_cast<int>(temp_hull.size()) - 1; ithull++)
	{
