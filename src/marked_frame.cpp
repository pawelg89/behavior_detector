...
{	
	if(abs(Points[(int)Points.size() - 1].x - Points[0].x) < 15 && abs(Points[(int)Points.size() - 1].y - Points[0].y) < 15)
	{	
		min_x = 10000;
		max_x = 0;
		min_y = 10000;
		...
		{
			line((Mat)image,Points[i],Points[(i+1)%(int)Points.size()],cvScalar(0,0,255),1,8,0);
		}
		CvPoint point;
		for(int i = 0; i < (int)Points.size(); i++)
		{
...
bool marked_frame::is_inside(detected_object* object, std::vector<cv::Point2f> Points)
{
	...
	for(int x=0; x<width; x++)
	{
		...
		{	
			while(mask.at<unsigned char>(Point(x,y)) == 255 && point_inside == false) 
			{
				y++;
				point_inside = true;
			}

			while(point_inside == true && mask.at<unsigned char>(Point(x,y)) == 255)
			{
				y++;
				point_inside = false;
			}
