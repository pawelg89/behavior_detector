...
void MouseEvent(int ev, int x, int y, int flags, void* param)
{
	...
	case CV_EVENT_RBUTTONDOWN:
		...
		{
			if((vec[i].x > (x-15) && vec[i].x < (x+15)) && (vec[i].y > (y-15) && vec[i].y < (y+15)))
			{
...
//Zmienne globalne dla w¹tku zbierajacego ramki z kamery IP
//static IplImage* buffer = NULL;
//CRITICAL_SECTION critical;

void ClearPoint(CvPoint *p)		//zerowanie wybranego punktu
{
	p->x = -1;
	p->y = -1;
}
int DI_LOG(string msg, int level, int log_level = 1)
{
	ofstream log;
	if(level <= log_level)
	{
...
//	char bufferChar[10];
	...
	int mode;
	if(!load_data("parameters.txt","mode",mode))
		mode = 5;

	cout<<"Chosen moode: "<<mode<<endl;
	char temp_msg[100] = {"Chosen mode: "};
  std::string temp_mode = {std::to_string(mode).substr(10)};
  //char temp_mode[10];
	...
	{
		throw (string) "DiffImage: Couldn't open video file. Check path: "+file_name;
	}
	...
	BackgroundSubtractorMOG2* bg = new BackgroundSubtractorMOG2(10000, mogThreshold,true);
//	bg->fTau = 0.1;
  ...
  Mat fore;
	...
	do
	{
		...
		if(convex->is_background_ok(frame->width,frame->height) && _char!='r' && _char!='R' /*&&
			...
			frameCounter!= 12020*/)
		 {
			...
			cvPutText(frame,std::to_string(frameCounter).c_str() , cvPoint(10,10),&font,cvScalar(0,0,255));
			frameCounter++;
		 ...
		 //else
		 //{
			// cvPutText(frame,itoa(frameCounter,bufferChar,10) , cvPoint(10,10),&font,cvScalar(0,0,255));
			// frameCounter++;
			// delete bg;//>~BackgroundSubtractorMOG2(); 
			// bg = new BackgroundSubtractorMOG2(10000, mogThreshold,true);
	...
	DI_LOG("Leaving main loop. Calling destructors...", 1);
	cvReleaseCapture(&video);
	cvDestroyAllWindows();
	cout << endl;
}

void load_marker_coord(vector<vector<Point2f>>&marker_coord, int camera_number)
{
	ifstream file2;
...
{	
  ...
  std::string temp_mode = std::to_string(mode);
	...
	for (int i=0; i < nbrViews; i++)
	{
		...
		if(mode != 7)
		{
			namedWindow(tempDir,CV_WINDOW_NORMAL);
			if(i==0)moveWindow(tempDir,7,1);
			if(i==1)moveWindow(tempDir,660,1);
			if(i==2)moveWindow(tempDir,7,500);
			if(i==3)moveWindow(tempDir,660,500);
			resizeWindow(tempDir,640,480);
		}
		if(i==0)
		{
	...
	//if(nbrViews>0) GardzinCapture->QueryFrame(frame,0);
	//else throw "asd";
	...
	do
	{
      ...
      if (frame.empty()) {
        ...
        std::cout << "!is_background_ok(frame.cols,frame.rows)" << std::endl;
			...
			//	bg_vec[c] = new BackgroundSubtractorMOG2(10000, mogThreshold,true);
			//	cout<<"Background reset"<<endl;
			//}

			TrackObjects3D(frame,convex_vec,marker_coord,c);
				
			//RestrictedAreaCheck(new IplImage(frame),vec,c,H_f,marker_coord,convex_vec);

      
			QueryPerformanceCounter(reinterpret_cast<LARGE_INTEGER *>(&stop));
			ms = (static_cast<long double>(stop)-start)/freq * 1000;
			if(mode != 7)
				imshow(pathDir[c],frame);
			//if(c<nbrViews-1)
			//{
			//	if(ms > 40.0)
			//	{
			...
			if(_char == 'p')
				if(waitTime == 1)
					...
					waitTime = 1;
      std::cout << "Getting frame:" << frameCounter++ << std::endl;
		}    
...
void match_objects(vector<vector<Point2f>> obj_positions, vector<vector<double>> &matches, int view, vector<double> path)
{
...
{	
	...
	do
	{
		for(int i = 0; i < n-1; i++)
		{
			if(matches[i][matches[i].size()-1] > matches[i+1][matches[i].size()-1])
			{
...
{	
	...
	for(int i = 0; i < (int)matches.size(); i++)
	{
		...
		if(is_labeled == matches[i].size()-1)
		{
			for(int j = 0; j < (int)matches[i].size()-1; j++)
			{
				blob_vec[j]->detected_objects[(size_t)matches[i][j]]->number = object_id;
			}
			object_id++;
		...
		{	
			...
			for(int j = 0; j < (int)matches[i].size()-1; j++)
			{
				if(blob_vec[j]->detected_objects[(size_t)matches[i][j]]->number < label && blob_vec[j]->detected_objects[(size_t)matches[i][j]]->number > -1)
				{
...
void match_text(vector<vector<double>> &matches, vector<Convex*> blob_vec, int a, int b)
{
	for(int i = 0; i < (int)blob_vec[a]->detected_objects.size(); i++)
	{	
		for(int j = 0; j < (int)blob_vec[b]->detected_objects.size(); j++)
		{	
			if( blob_vec[b]->detected_objects[j]->number > -1 && blob_vec[a]->detected_objects[i]->number > -1)
			{
				if(blob_vec[a]->detected_objects[i]->number == blob_vec[b]->detected_objects[j]->number)
				{
					
					for(int beh = 0; beh < (int)blob_vec[a]->detected_objects[i]->bFilter.size(); beh++)
					{
						if(blob_vec[a]->detected_objects[i]->bFilter[beh]->found == true || blob_vec[b]->detected_objects[j]->bFilter[beh]->found == true)
						{
...
void DiffImage::TrackObjects3D(Mat Frame, vector<Convex*> blob_vec, vector<vector<Point2f>>& marker_coord, int view)
{	
	static vector<Mat> H;
...
//	static int object_id = 1;
	...
	if(first_run)
	{
		vector<Point2f> obj_corners(4); // wspó³rzêdne zewnêtrznych wierzcho³ków markera z pliku!!!
	...
	for(int c = 0; c < (int)blob_vec.size(); c++)
	{
		...
		{	
			orginal_positions.push_back((Point2f)blob_vec[c]->detected_objects[i]->KFilter->measPt);
		}
	...
	if(match_possible)
	{
		...
		//////////////////////////////////////////////////////////usuwanie tych samych id
		for(int j = 0; j < (int)matches.size(); j++)
		{	
			for(int i = 0; i < (int)matches[j].size()-1; i++)
			{	
				view = i;
				int label = blob_vec[view]->detected_objects[(size_t)matches[j][view]]->number;
				for(int o = 0; o < (int)blob_vec[view]->detected_objects.size(); o++)
				{
					if(label == blob_vec[view]->detected_objects[o]->number && o != matches[j][view])
					{
		...
		{	
			...
			k_filter.resize(blob_vec[0]->detected_objects[(size_t)matches[0][0]]->bFilter.size(),false);

			for(int j = 0; j < (int)matches[i].size()-1; j++)
			{	
				
				for(int k = 0; k < (int)blob_vec[j]->detected_objects[(size_t)matches[i][j]]->bFilter.size(); k++)
				{
					if(blob_vec[j]->detected_objects[(size_t)matches[i][j]]->bFilter[k]->found == true)
					{
						k_filter[k] = true;
					}
				}
			...
			for(int j = 0; j < (int)matches[i].size()-1; j++)
				{	
					for(int k = 0; k < (int)blob_vec[j]->detected_objects[(size_t)matches[i][j]]->bFilter.size(); k++)
					{
						blob_vec[j]->detected_objects[(size_t)matches[i][j]]->bFilter[k]->found = k_filter[k];
					}
				}
			

		}
		//////////////////////////////////////////////////////////////////////////////////////////////przepisywanie is inside
		for(int i = 0; i < (int)matches.size(); i++)
		{
			bool is_inside = false;
			for(int j = 0; j < (int)matches[i].size()-1; j++)
			{
				if(blob_vec[j]->detected_objects[(size_t)matches[i][j]]->is_inside_restricted_area == true)
				{
					is_inside = true;
					break;
				}
			...
			if(is_inside)
			{
				for(int j = 0; j < (int)matches[i].size()-1; j++)
				{
					blob_vec[j]->detected_objects[(size_t)matches[i][j]]->is_inside_restricted_area = true;
				}
			}
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////przepisywanie tekstu gdy w mniej niz N widokach
	...
	for(int i = 0; i < (int)blob_vec.size(); i++)
	{
			...
			{
				if(i!=j)
					match_text(matches,blob_vec,i,j);
			}
	}
...
void DiffImage::RestrictedAreaCheck(IplImage* frame, vector<Point2f> &vec,int camera, vector<Mat>H_f,vector<vector<Point2f>>& marker_coord,vector<Convex*> blob_vec)
{
	...
	if(camera == 0)
	{
		...
		if((int)vec.size() <2 && (int)vec.size() != MFrame->points_counter) MFrame->points_counter = (int)vec.size();
		if(vec.size() >1)
		{	
		...
		if(MFrame->shape_closed)
		{
			...
			{
				if(!blob_vec[camera]->detected_objects[i]->human  && (int)blob_vec[camera]->detected_objects[i]->KFilter->posv.size() > 5)
				{
					if(MFrame->is_inside(blob_vec[camera]->detected_objects[i],vec))// || blob_vec[camera]->detected_objects[i]->is_inside_restricted_area)
					{
					...
					//	{	
						//	cout << '\a' << endl << "\b";
							blob_vec[camera]->detected_objects[i]->is_inside_restricted_area = true;
							blob_vec[camera]->detected_objects[i]->inside_counter++;
					//	}
					}
					else
					{
						if(blob_vec[camera]->detected_objects[i]->outside_counter++ > outside_thresh)
						{
							blob_vec[camera]->detected_objects[i]->is_inside_restricted_area = false;
		...
		if(vec.size() > 0)
		{
			...
			if(MFrame->shape_closed)
			{
				for(int i = 0; i < (int)blob_vec[camera]->detected_objects.size(); i++)
				{
					if(!blob_vec[camera]->detected_objects[i]->human  && (int)blob_vec[camera]->detected_objects[i]->KFilter->posv.size() > 5)
					{
						if(MFrame->is_inside(blob_vec[camera]->detected_objects[i],final))//|| blob_vec[camera]->detected_objects[i]->is_inside_restricted_area)
						{
							blob_vec[camera]->detected_objects[i]->outside_counter = 0;
					//		rectangle((Mat)frame,blob_vec[camera]->detected_objects[i]->rect,cvScalar(0,255,255),2);
						//	if(!blob->detected_objects[i]->is_inside_restricted_area)
